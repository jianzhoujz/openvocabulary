...
针对 PTE Core 与 TCF Canada 两门语言考试的词表，共三份：
PTE 一份，TCF 按条目词数拆成单词、短语两份。不是通用词汇书——每一条都绑定具体考试的具体题型。

仓库根目录是基于这三份词表的背单词网页，见 [根 README](../README.md)。

| 词表 | 考试 | 目标 | 条目数 | 对照语言 |
|---|---|---|---|---|
| [`pte-core/`](pte-core/) | PTE Core（英语） | CLB 9 | 1266 | 英 → 中 |
| [`tcf-canada-mots/`](tcf-canada-mots/) | TCF Canada（法语） | NCLC 7+ | 435 | 法 → 英 → 中 |
| [`tcf-canada-phrases/`](tcf-canada-phrases/) | TCF Canada（法语） | NCLC 7+ | 678 | 法 → 英 → 中 |

拆分规则：去掉冠词后只剩一个词的条目进单词表，两个及以上的进短语表。
`la résidence permanente` 这类复合词按词数算在短语表；
`CONJUG` / `GENRE` 的中文标题规则条也在短语表，单个词的变位与词尾规则在单词表。
两表相加 1113 条，与拆分前总数一致。
...
不是生僻词。所以短语表里大量条目是整句而非单词，标记为 `chunk` / `expr.`，要整条背。
...
**收录的不只是词。** 影响分数的不只是词汇量，所以词表还包含语法条目：
英语侧的 `RA`（Read Aloud 的重音位置、静音字母、-ed 读法、弱读与意群停顿，60 条）；
法语侧的 `CONJUG`（核心动词全套变位、时态对立、虚拟式触发词）和
`GENRE`（阴阳性词尾规则与易错词），按条目词数拆进了单词、短语两表。
...
各份 README 里的分数对照表按公开资料整理，**换算标准会调整，用之前请到考试官方页面
核对一次**。
